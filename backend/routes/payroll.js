...
const calculatePayroll = async (workerId, from, to, advance, settings, { allowZeroHours = false } = {}) => {
  ...
  if (payType === 'salary') {
    ...
    return {
      ...
      rate: 0,
      hours: totalHours,
      normalPay: Math.round(gross * 100) / 100,
  ...
  return {
    ...
    rate,
    hours: totalHours,
    normalPay: Math.round(regularGross * 100) / 100,
...
router.get('/my', requireAuth, requireWorker, async (req, res) => {
  try {
    ...
    res.json(payrolls.map(p => ({
      ...
      normalPay: Math.round((Number(p.gross) - Number(p.holiday_pay || 0) - Number(p.holiday_accrual_pay || 0)) * 100) / 100,
